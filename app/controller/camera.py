...
def upload_image():
    ...
    # Save raw image to MongoDB (or just file path)
...
def process_latest():
    doc = database.image_collection.find_one({"processed": False}, sort=[("timestamp", -1)])
    if not doc:
        return jsonify({"error": "No unprocessed image found"}), 404

    raw_bytes = base64.b64decode(doc["image_raw_base64"])
    img = cv2.imdecode(np.frombuffer(raw_bytes, np.uint8), cv2.IMREAD_COLOR)
    if img is None:
        return jsonify({"error": "Failed to decode image"}), 400

    # Resize for YOLO (keep 416x416 for accuracy)
    ...
    # Update original doc
    database.image_collection.update_one(
        {"_id": doc["_id"]},
        {
            "$set": {
